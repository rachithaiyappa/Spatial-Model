...
%Triad interaction has been included
tic
...
while samp_col < size(samp_time,2)
    ...
    while min_fish_time < samp_time(samp_col)
        ...
        %Triad interaction
            ...
            %incorporating triad interaction
            delta_phi(1) = abs(phi(idx,col)-phi(idx_d(2),col));
            delta_phi(1) = abs(wrapToPi(delta_phi(1)));
            delta_phi(2) = abs(phi(idx,col)-phi(idx_d(3),col));
            delta_phi(2) = abs(wrapToPi(delta_phi(2)));
            delta_phi(3) = abs(phi(idx_d(2),col)-phi(idx_d(3),col));
            delta_phi(3) = abs(wrapToPi(delta_phi(3)));
            
            %identifying majority
            [~,maj_pair_idx] = min(delta_phi);
            if maj_pair_idx == 1
                temp_idx_1 = idx;
                temp_idx_2 = idx_d(2);
                temp_foc_idx = idx_d(3);
            elseif maj_pair_idx == 2
                temp_idx_1 = idx;
                temp_idx_2 = idx_d(3);
                temp_foc_idx = idx_d(2);
            else
                temp_idx_1 = idx_d(2);
                temp_idx_2 = idx_d(3);
                temp_foc_idx = idx;
            ...
            rand_num = rand;
            if rand_num > 0.5
                phi(temp_foc_idx,col) = phi(temp_idx_1,col);
            else
                phi(temp_foc_idx,col) = phi(temp_idx_2,col);
            end
